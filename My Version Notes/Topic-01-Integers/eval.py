...
def eval(s): 
    ...
    if s[0] == '-':
        sign = -1
        s = s[1:] ## idiom that says dro the first character & give the rest of the sequence
    else:
        sign = 1
    multi = 1.0
    fractional = False
...
def test_eval(): 
    ...
    assert eval("0001") == 1
    assert eval("-99") == -99 ## practicing negation
    assert eval("1.") == 1
    assert eval("1.23") == 1.23
    assert eval("-1.23") == -1.23
    try:
        eval("1..2")
        assert False, "No error [1..2]"
    except Exception as e: 
        print("got an error for [1..2]")

    try: 
        eval(" 1")
        assert False, "No error [ 1]"
    except Exception as e: 
